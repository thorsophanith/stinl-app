...
<div class="flex items-center justify-between px-3 animate-slide-down">
    ...
    <div class="flex gap-3">
        <a href="{{ route('parameter-prices.create') }}" class="px-4 py-1.5 bg-blue-500 hover:bg-blue-600 ease-in text-white rounded-md duration-300 ring-2 mb-1 text-xs md:text-sm font-medium transform hover:scale-105 transition">Add Price List</a>
    </div>
</div>

<div class="bg-white rounded-lg shadow-md p-8 animate-fade-in-up">
    <form method="GET" action="{{ route('parameter-prices.index') }}" class="flex flex-col sm:flex-row items-center justify-between mb-4 gap-4">
        <input
            type="text"
            name="search"
            placeholder="Search..."
        ...
        <div class="flex items-center gap-2">
            ...
            <select id="page-size" name="per_page" class="border rounded-md px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-500 transition" onchange="this.form.submit()">
        ...
        <table class="min-w-full">
            <thead class="bg-gray-100 rounded-md">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Parameter</th>
                    <th class="px-6 py-4 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Test Duration</th>
                    <th class="px-6 py-4 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Lab Type</th>
                    <th class="px-6 py-4 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Price</th>
                ...
                @forelse ($prices as $price)
                    <tr class="hover:bg-blue-50 transition ease-out duration-300">
                        ...
                        <td class="py-2 text-gray-700 flex gap-2 justify-center items-center text-xs lg:w-[90px]">
                            <a href="{{ route('parameter-prices.edit', $price->id) }}" class="px-4 py-1 bg-blue-500 hover:bg-blue-600 ease-in text-white rounded-md duration-300 ring-2 mb-1 font-medium transform hover:scale-105">Edit</a>
                            <form action="{{ route('parameter-prices.destroy', $price->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete all standards with code {{ $price->id }}?');">
                                @csrf
                                @method('DELETE')
                                <button type="button" onclick="showDeleteDialog(this.form)" class="px-4 py-1 bg-red-500 hover:bg-red-600 ease-in text-white rounded-md duration-300 ring-2 ring-red-300 mb-1 font-medium transform hover:scale-105">Delete</button>
                            </form>
                        </td>
                    </tr>
                @empty
...
@if ($errors->any())
    ...
    <div id="errorDialog" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 animate-fade-in">
